...
# --- Draw arrow for gene
def draw_arrow(additional_tabs, X, Y, L, l, H, h, strand, color, color_contour, category, gid, domain_list, pfam_text):
    """
    SVG code for arrow:
    ...
    for domain in domain_list:
        ...
        if strand == "+":
            ...
            if (dX + dL) < head_start + collision_x - x_margin_offset:
                ...
                arrow += "fill=\"rgb({})\" stroke=\"rgb(0,0,0)\" ".format(",".join([str(val) for val in dcolor]))
                arrow += "stroke-width=\"{}\" opacity=\"0.75\" />\n".format(str(domain_contour_thickness))
                if re.match("PF", dacc) != None:
                    arrow += "<text x=\"{}\" y=\"45\" text-anchor=\"middle\" fill=\"black\" font-size=\"15\">{}</text>\n".format( str(X+dX + 0.5*dL), pfam_text[dacc])
            else:
                del points[:]
                ...
                av_xs = 0.5*(min(xs)+max(xs))
                #av_ys = 0.5*(min(ys)+max(ys))
                ...
                arrow += "stroke=\"rgb(0,0,0)\" "
                arrow += "stroke-width=\"{}\" opacity=\"0.75\" />\n".format(str(domain_contour_thickness))
                if re.match("PF", dacc) != None:
                    arrow += "<text x=\"{}\" y=\"45\" text-anchor=\"middle\" fill=\"black\" font-size=\"15\">{}</text>\n".format(str(av_xs), pfam_text[dacc])
...
def SVG(write_html, outputfile, GenBankFile, BGCname, pfdFile, use_pfd, color_genes, color_domains, pfam_domain_categories, pfam_info, loci, max_width, pfam_text, H=30, h=15, l=30, mX=10, mY=10, scaling=30, absolute_start=0, absolute_end=-1):
    '''
    ...
    for seq_record in records:
        ...
        for feature in [feature for feature in seq_record.features if feature.location.start >= absolute_start and feature.location.end <= absolute_end]:
            if feature.type == 'CDS':
                ...
                arrow = draw_arrow(additional_tabs, start+mX, add_origin_Y+mY+h, int(feature.location.end-feature.location.start)/scaling, l, H, h, strand, color, color_contour, gene_category, cds_tag, identifiers[identifier], pfam_text)
                if arrow == "":
